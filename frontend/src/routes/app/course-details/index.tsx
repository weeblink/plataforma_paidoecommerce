...
export default function CourseDetailsPage() {
  ...
  const courseId = useLoaderData() as number
  ...
  async function fetchData() {
    ...
    try {
      ...
      if( !data.data.is_pay ){
        await api.post(`/courses/${data.data.id}/check-free-course`);
      }
  ...
  function getNextClass() {
    ...
    const currentModule = course.modules[currentModuleIndex === -1 ? 0 : currentModuleIndex]
  ...
  return (
    <div>
      <BreadcrumbLinks courseName={course.title} />
